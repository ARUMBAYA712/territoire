...
| `15_elus.py` | 2 | Élus locaux — trois défauts de lecture corrigés |
...
1. ~~Renseigner `MENTIONS`~~ — fait le 8 septembre 2026, `04_generation.py`
   version 23. La page `/mentions-legales/` est produite.
2. ~~Protéger `/Terri_Admin/` par mot de passe~~ — fait. Vérifier que
   `chiffrer.php` a bien disparu du dépôt comme du serveur.
3. Supprimer `administration/index.html` du dépôt, sinon l'ancienne page est
   servie à la place du leurre. `git rm --cached administration/index.html` :
   le générateur le supprime localement, mais un fichier suivi par Git
   revient au déploiement suivant.
...
**Le dossier `Terri_Admin/Documents/`** reçoit la documentation du projet,
relisible depuis un téléphone par `documents.php`. Il est protégé par mot
de passe mais servi par le web : aucun secret ne doit y être déposé — ni
`.htpasswd`, ni clé, ni identifiant. Toute modification d'un document du
projet doit y être reportée en même temps que dans le dépôt.
