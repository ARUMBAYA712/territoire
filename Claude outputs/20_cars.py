...
def colonnes_de(archive, nom):
    """Noms de colonnes d'un fichier du GTFS, dans l'ordre."""
    for ligne in lire_table(archive, nom):
        return list(ligne.keys())
    return []
...
def inventaire_des_lignes(archive, services):
    ...
    return [(noms.get(rid, ("", ""))[0], noms.get(rid, ("", ""))[1], n, rid)
            for rid, n in compte.items()]


def famille_de(court):
    """Préfixe alphabétique d'un numéro de ligne : « CSA05 » → « CSA ».

    Les réseaux départementaux codent presque toujours leurs services
    par secteur ou par nature — c'est ce regroupement qui rend une
    liste de cinq cents lignes lisible d'un coup d'œil.
    """
    tete = ""
    for caractere in str(court or "").strip():
        if caractere.isalpha():
            tete += caractere
        else:
            break
    return tete.upper() or "(sans lettre)"
...
def main():
    ...
    with archive:
        ...
        if "--inventaire" in sys.argv:
            # Les arrêts d'abord : la liste qui compte n'est pas celle
            # des lignes du département, c'est celle des lignes qui
            # passent CHEZ NOUS. Une trentaine, qu'on lit d'un trait.
            arrets, _examines = rattacher_arrets(archive, contours, par_code)
            _p, lignes_ici, noms = desserte(
                archive, arrets, services_mardi, set())
            chez_nous = set()
            for ensemble in lignes_ici.values():
                chez_nous |= ensemble

            total_courses = sum(n for _c, _l, n, _r in inventaire)
            print(f"\n  {len(inventaire)} ligne(s) circulent ce mardi, "
                  f"pour {total_courses} course(s).")

            # 1 — la distribution. Une ligne régulière fait une dizaine
            # d'allers-retours ; un service scolaire en fait deux.
            tranches = [(1, 2), (3, 5), (6, 10), (11, 10 ** 6)]
            print("\n  Répartition des lignes selon leur nombre de courses :")
            for bas, haut in tranches:
                lot = [x for x in inventaire if bas <= x[2] <= haut]
                if not lot:
                    continue
                libelle = f"{bas} à {haut}" if haut < 10 ** 6 else f"{bas} et +"
                print(f"    {libelle:>9} course(s) : {len(lot):4} ligne(s), "
                      f"{sum(x[2] for x in lot):5} course(s)")

            # 2 — les familles de numérotation.
            familles = {}
            for court, _long, n, _rid in inventaire:
                f = famille_de(court)
                a, b = familles.get(f, (0, 0))
                familles[f] = (a + 1, b + n)
            print("\n  Familles de numéros, de la plus fournie à la moins :")
            for f, (lignes_n, courses_n) in sorted(
                    familles.items(), key=lambda kv: -kv[1][1])[:14]:
                exemple = next((c for c, _l, _n, _r in inventaire
                                if famille_de(c) == f), "")
                print(f"    {f:>12}  {lignes_n:4} ligne(s), {courses_n:5} "
                      f"course(s)   ex. {exemple}")

            # 3 — la seule liste sur laquelle se décide le filtre.
            print(f"\n  LIGNES DESSERVANT LE TERRITOIRE ({len(chez_nous)}) :\n")
            for court, long_, n, rid in inventaire:
                if rid not in chez_nous:
                    continue
                print(f"    {n:4} course(s)  {court:12} {long_[:52]}")

            # 4 — ce que le fichier offre d'autre pour trancher.
            print("\n  Colonnes disponibles, au cas où l'une distinguerait "
                  "le scolaire :")
            for fichier in ("routes.txt", "trips.txt"):
                print(f"    {fichier:14} {', '.join(colonnes_de(archive, fichier))}")

            print("\n  Envoyez cette sortie : le filtre se règle sur la "
                  "liste des lignes du territoire,\n  pas sur les plus "
                  "fournies du département.\n")
            return

        scolaires = {court for court, long_, _n, _r in inventaire
