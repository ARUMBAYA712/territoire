...
C'est faisable. Les débits mensuels sont **déjà calculés par l'API**,
les températures mensuelles **déjà calculées par Météo-France** et
remontent à 1988 pour un poste situé à **Chatte**, dans la vallée, et le
poids d'un graphique est **cinq fois inférieur** à celui d'une de nos
cartes.

Mais la partie « débits » de ce rapport a dû être corrigée deux fois,
et c'est l'enseignement principal : **sur ces sources, la profondeur
d'une série ne dit rien de sa valeur**. Une chronique de cinquante-sept
ans peut être inexploitable sur ses trente premières années sans que
rien ne le signale, sinon un champ de qualification qu'il faut penser à
lire. Le vrai sujet n'est ni la donnée ni la technique : c'est de ne
pas faire dire à une courbe plus qu'elle ne dit.
...
### Débit des rivières — vérifié, et deux fois corrigé

> **Cette section a été réécrite le 8 septembre au soir.** Elle
> contenait deux recommandations fausses, l'une après l'autre. Les
> vérifications qui suivent ont toutes été faites sur l'API elle-même,
> valeur par valeur — c'est ce qui aurait dû être fait du premier coup.

Hub'Eau, API hydrométrie v2, sur l'emprise `bbox=5.05,44.95,5.60,45.30` :
**22 stations**, dont sept sur des communes du canton. L'API sert
directement le **débit moyen mensuel** (`grandeur_hydro_elab=QmM`), déjà
calculé, avec pour chaque valeur son statut et sa **qualification**.

#### Ce qui a été corrigé, et pourquoi cela compte

**Première erreur : le débit naturel reconstitué de l'Isère.** Je
recommandais de l'afficher, en supposant que l'API le publiait. La
station `W320001201` (« QNR Le Port ») existe au référentiel et **ne
diffuse aucune valeur** — ni mensuelle ni journalière. Votre propre
exécution le signalait : « aucune mesure ».

**Seconde erreur : La Bourne comme série de référence.** Corrigeant la
première, j'ai mis en avant la station EDF de la Bourne à
Saint-Just-de-Claix et ses **708 valeurs mensuelles depuis 1967**. La
profondeur était réelle. Le contenu ne l'était pas :

| Mois | Valeur servie | Qualification |
|---|---|---|
| janvier 1967 | 947 L/s — soit 0,9 m³/s | **Non qualifiée** |
| janvier 1990 | 1 542 L/s | Bonne |
| janvier 2020 | 8 536 L/s | Bonne |
| mars 1967 | 2 356 L/s | **Non qualifiée** |
| mars 2020 | 25 349 L/s | Bonne |

Les valeurs anciennes sont environ **dix fois trop faibles**, de façon
constante d'un mois à l'autre. Ce n'est pas de l'hydrologie : c'est une
rupture d'échelle ou d'unité dans l'archive. Tracées telles quelles,
elles auraient dessiné une hausse spectaculaire du débit de la Bourne
depuis cinquante ans — **une impression fausse appuyée sur des données
vraies**, exactement ce que ce site s'interdit, et cette fois dans un
graphique produit par nous.

**Un troisième piège, trouvé au passage.** Le même site hydrométrique
porte deux stations simultanées, l'une EDF, l'autre DREAL. En janvier
2003, l'une annonce 0,9 m³/s et l'autre **20,5 m³/s** pour la même
rivière au même endroit : elles ne mesurent pas la même chose. Or le
collecteur interroge les observations **par site**, ce qui les mélange.
Cela ne se voit pas sur l'indicateur « proche des valeurs habituelles »,
qui compare chaque mois à lui-même, mais c'est une fragilité à traiter.
Les chroniques, elles, interrogent désormais la **station**.

#### Ce qu'il faut retenir pour la suite

**La profondeur d'une série ne dit rien de sa valeur.** Sur ces
sources, le bon critère n'est pas « depuis quand » mais « à partir de
quand la donnée est qualifiée ». Le collecteur applique donc deux
filets :

1. **la qualification, qui vient de la source** — les mois « Non
   qualifiée » sont écartés, le producteur n'ayant pas expertisé la
   valeur ;
2. **un contrôle d'homogénéité, qui ne vient de personne** — la médiane
   des cinq premières années est comparée à celle des cinq dernières ;
   au-delà d'un rapport de trois, la série entière est refusée et le
   motif écrit dans la sortie du script.
...
**Quelle série sera publiée, alors ?** Celle qui survit aux deux
filets, et la plus longue d'entre elles. Sur ce territoire, ce sera
vraisemblablement l'Isère à Saint-Gervais, dont les valeurs de 1969
(269 m³/s en janvier) sont du bon ordre de grandeur. Elle portera sa
réserve écrite sur le graphique : **l'Isère est fortement aménagée**,
et son débit traduit autant la gestion des ouvrages que la pluie et la
fonte des neiges. La courbe décrit la rivière telle qu'elle coule, non
le climat du bassin.

Le décompte définitif ne sera connu qu'après la première collecte : le
script écrit combien de mois ont été retenus, combien écartés, et
lesquels.
...
- **La part réellement qualifiée de chaque série de débit** : elle ne
  se saura qu'à la première collecte, station par station.
